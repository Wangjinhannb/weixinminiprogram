...
function validateBookingDate(date) {
  if (!isValidDateString(date)) {
    var err = new Error('预约日期格式不正确，请使用 YYYY-MM-DD');
    err.code = 400;
    throw err;
  }
  var today = new Date();
...
function validateBookingTimeRange(startTime, endTime) {
  if (!/^([01]\d|2[0-3]):([0-5]\d)$/.test(String(startTime || '').trim()) || !/^([01]\d|2[0-3]):([0-5]\d)$/.test(String(endTime || '').trim())) {
    var err = new Error('预约时间格式不正确，请使用 HH:mm');
    err.code = 400;
    throw err;
  }
  if (timeToSeconds(endTime) <= timeToSeconds(startTime)) {
    var err2 = new Error('结束时间必须晚于开始时间');
    err2.code = 400;
    throw err2;
  }
  return {
    startTime: normalizeTimeString(startTime, false),
    endTime: normalizeTimeString(endTime, false),
...
module.exports = {
  ...
  validateBookingDate: validateBookingDate,
  validateBookingTimeRange: validateBookingTimeRange
};
